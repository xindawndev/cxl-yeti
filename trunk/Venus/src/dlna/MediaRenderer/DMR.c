...
void DMRDestroyFromChain(DMR instance)
{
    if(instance != NULL)
    {
        ...
        if(state != NULL)
        {
            ...
            String_Destroy(state->PlayMedia);
            String_Destroy(state->RecMedia);
            String_Destroy(state->RecQualityModes);
...
DMR DMR_Method_Create(void* chain, unsigned short port, char* friendlyName, char* serialNumber, char* UDN, char* protocolInfo, ILibThreadPool threadPool, int open2thirdparty)
{
    ...
    state->PlayMedia = String_Create("NOT_IMPLEMENTED");;
    state->RecMedia = String_Create("NOT_IMPLEMENTED");;
    state->RecQualityModes = String_Create("NOT_IMPLEMENTED");
    state->port = port;
    ...
    state->DMR_microStack = DMR_CreateMicroStack(state->DMR_microStackChain, state->FriendlyName, state->udn, state->SerialNumber, state->seconds, state->port, open2thirdparty);
...
DMR_Error DMR_Method_SetDeviceCapabilities(DMR instance, const char * PlayMedia, const char * RecMedia, const char * RecQualityModes)
{
    ...
    if ( istate->PlayMedia )
    {
        String_Destroy( istate->PlayMedia );
        istate->PlayMedia = String_Create( PlayMedia );
    }

    if ( istate->RecMedia )
    {
        String_Destroy( istate->RecMedia );
        istate->RecMedia = String_Create( RecMedia );
    }

    if ( istate->RecQualityModes )
    {
        String_Destroy( istate->RecQualityModes );
        istate->RecQualityModes = String_Create( RecQualityModes );
    }
...
DMR_Error DMR_StateChange_CurrentTransportActions(DMR instance, unsigned short allowedActions)
{
    ...
    if(allowedActions < 1 || allowedActions > 63)
    {
        return DMR_ERROR_INVALIDARGUMENT;
    }
    istate = (DMR_InternalState)instance->internal_state;
...
DMR_Error DMR_StateChange_CurrentTrackMetaData(DMR instance, struct CdsObject* trackMetadata)
{
    ...
    {
        String_Destroy(istate->CurrentTrackMetaData);
        istate->CurrentTrackMetaData = String_Create("");
        istate->LastChangeMask |= EVENT_CURRENTTRACKMETADATA;
    }
    DMR_Unlock(instance);
...
DMR_Error DMR_StateChange_AVTransportURIMetaData(DMR instance, struct CdsObject* metadata)
{
    ...
    {
        String_Destroy(istate->AVTransportURIMetaData);
        istate->AVTransportURIMetaData = String_Create("");
        istate->LastChangeMask |= EVENT_AVTRANSPORTURIMETADATA;
    }
    DMR_Unlock(instance);
