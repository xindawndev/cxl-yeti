...
struct DMR__StateVariable_AVTransport_A_ARG_TYPE_SeekMode DMR__StateVariable_AVTransport_A_ARG_TYPE_SeekMode_Impl =
{
    ...
    4519,
    19,
    {"TRACK_NR", "ABS_TIME", "REL_TIME", NULL},
    4538,
    16
};
...
void DMR_StreamDescriptionDocument(struct ILibWebServer_Session *session, int intelnalerror);
...
void DMR_Init(struct DMR_DataObject *state, void *chain, const int NotifyCycleSeconds,const unsigned short PortNumber)
{
    ...
    for(i=0;i<state->AddressListLength;++i)
    {
        ...
        if ( !state->NOTIFY_RECEIVE_socks[i] )
            continue;//leochen
        ILibAsyncUDPSocket_JoinMulticastGroup(state->NOTIFY_RECEIVE_socks[i], state->AddressList[i], inet_addr(UPNP_GROUP));
    }
}
...
#define DMR_Dispatch_ConnectionManager_GetCurrentConnectionIDs(buffer,offset,bufferLength, session)\
{\
    if(DMR_FP_ConnectionManager_GetCurrentConnectionIDs == NULL)\
    DMR_Response_Error(session,501,"No Function Handler");\
    else\
    DMR_FP_ConnectionManager_GetCurrentConnectionIDs((void*)session);\
}
...
#define DMR_Dispatch_ConnectionManager_GetProtocolInfo(buffer,offset,bufferLength, session)\
{\
    if(DMR_FP_ConnectionManager_GetProtocolInfo == NULL)\
    DMR_Response_Error(session,501,"No Function Handler");\
    else\
    DMR_FP_ConnectionManager_GetProtocolInfo((void*)session);\
}
    ...
    LVL3DEBUG(errorPacketLength=ILibGetRawPacket(header,&errorPacket);)
        ...
        else if(header->DirectiveLength==3 && memcmp(header->Directive,"GET",3)==0)
        {
            if(header->DirectiveObjLength==1 && memcmp(header->DirectiveObj,"/",1)==0)
            {
                ...
                // not open to thirdparty, check key header
                if (DMR__Device_MediaRenderer_Impl.open2thirdparty == 0)
                {
                    keyheader = ILibGetHeaderLine(header, "PPTVProduct", 11);
                    if (keyheader != NULL/* && strcmp(keyheader, "1") == 0*/)
                    {
                        DMR_StreamDescriptionDocument(session, 1);
                    }
                    else
                    {
                        DMR_StreamDescriptionDocument(session, 0);
                    }
                }
                else
                    DMR_StreamDescriptionDocument(session, 1);
...
void DMR_MasterPreSelect(void* object,void *socketset, void *writeset, void *errorset, int* blocktime)
{
    ...
    if(DMR_Object->UpdateFlag!=0)
    {
        ...
        {
            if (DMR_Object->NOTIFY_SEND_socks[i] == NULL)
                continue;
        ...
        for(i=0;i<DMR_Object->AddressListLength;++i)
        {
            if (DMR_Object->NOTIFY_RECEIVE_socks[i] == NULL)
                continue;
    ...
    if(done!=0 && ((struct SubscriberInfo*)subscriber)->Disposing==0)
    {
        ...
        if(((struct SubscriberInfo*)subscriber)->RefCount==0)
        {
            LVL3DEBUG(printf("\r\n\r\nSubscriber at [%s] %d.%d.%d.%d:%d was/did UNSUBSCRIBE while trying to send event\r\n\r\n",((struct SubscriberInfo*)subscriber)->SID,(((struct SubscriberInfo*)subscriber)->Address&0xFF),((((struct SubscriberInfo*)subscriber)->Address>>8)&0xFF),((((struct SubscriberInfo*)subscriber)->Address>>16)&0xFF),((((struct SubscriberInfo*)subscriber)->Address>>24)&0xFF),((struct SubscriberInfo*)subscriber)->Port);)
                DMR_DestructSubscriberInfo(((struct SubscriberInfo*)subscriber));
        }
        else if(header==NULL)
        {
...
DMR_MicroStackToken DMR_CreateMicroStack(void *Chain, const char* FriendlyName,const char* UDN, const char* SerialNumber, const int NotifyCycleSeconds, const unsigned short PortNum, int open2thirdparty)
{
    ...
    DMR__Device_MediaRenderer_Impl.open2thirdparty = open2thirdparty;
    ...
    if(DMR__Device_MediaRenderer_Impl.Manufacturer == NULL) {DMR__Device_MediaRenderer_Impl.Manufacturer = "PPTV Corporation";}
    if(DMR__Device_MediaRenderer_Impl.ManufacturerURL == NULL) {DMR__Device_MediaRenderer_Impl.ManufacturerURL = "http://www.pptv.com";}
    if(DMR__Device_MediaRenderer_Impl.ModelDescription == NULL) {DMR__Device_MediaRenderer_Impl.ModelDescription = "PPTV DLNA DMR Using Auto-Generated UPnP Stack";}
    if(DMR__Device_MediaRenderer_Impl.ModelName == NULL) {DMR__Device_MediaRenderer_Impl.ModelName = "PPTV DLNA DMR";}
    if(DMR__Device_MediaRenderer_Impl.ModelNumber == NULL) {DMR__Device_MediaRenderer_Impl.ModelNumber = "PPTV-X1";}
...
void DMR_StreamDescriptionDocument(struct ILibWebServer_Session *session, int intelnalerror)
{
    ...
    if (intelnalerror == 0)
    {
        ILibWebServer_StreamHeader_Raw(session,500,"PPTV DMR Internal Error",responseHeader,1);
        return;
    }
