...
#define YETI_AES_BLOCK_SIZE 16

static const YETI_UInt32 TE0[256] = {
...
static const YETI_UInt32 Te4[256] = {
...
static const YETI_UInt32 TD0[256] = {
...
static const YETI_UInt32 Td4[256] = {
...
+---------------------------------------------------------------------*/
#define RORc(x, y) \
    ( ((((YETI_UInt32)(x)&0xFFFFFFFFUL)>>(YETI_UInt32)(y)) | \
    ((YETI_UInt32)(x)<<(YETI_UInt32)(32-(y)))) & 0xFFFFFFFFUL)
...
#define YETI_BYTE(x,n) (((unsigned char)((x) >> (8 * (n)))))

/*----------------------------------------------------------------------
|   local utility functions
+---------------------------------------------------------------------*/
static YETI_UInt32 setup_mix(YETI_UInt32 temp)
{
    return (Te4_3[YETI_BYTE(temp, 2)]) ^
        (Te4_2[YETI_BYTE(temp, 1)]) ^
        (Te4_1[YETI_BYTE(temp, 0)]) ^
        (Te4_0[YETI_BYTE(temp, 3)]);
}

static YETI_UInt32 setup_mix2(YETI_UInt32 temp)
{
    return Td0(0xFF & Te4[YETI_BYTE(temp, 3)]) ^
        Td1(0xFF & Te4[YETI_BYTE(temp, 2)]) ^
        Td2(0xFF & Te4[YETI_BYTE(temp, 1)]) ^
        Td3(0xFF & Te4[YETI_BYTE(temp, 0)]);
}

static inline YETI_UInt32 LOAD32H(const YETI_UInt8* p)
{
    return ((((YETI_UInt32)p[0])<<24) |
        (((YETI_UInt32)p[1])<<16) |
        (((YETI_UInt32)p[2])<< 8) |
        (((YETI_UInt32)p[3])    ));
}

static inline void STORE32H(YETI_UInt32 x, YETI_UInt8* p)
{
    p[0] = (YETI_UInt8)(x>>24);
    p[1] = (YETI_UInt8)(x>>16);
    p[2] = (YETI_UInt8)(x>> 8);
...
{
public:
    AesBlockCipher(const YETI_UInt8* key,
        YETI_Size         key_size);
    YETI_Result process_block(const YETI_UInt8* , YETI_UInt8*) {
        return YETI_ERROR_INTERNAL;
    }
    YETI_Size   get_block_size() { return YETI_AES_BLOCK_SIZE; }
    Algorithm  get_algorithm() { return AES_128;            }

protected:
    YETI_UInt32   m_eK[60];
    YETI_UInt32   m_dK[60];
    YETI_Cardinal m_RoundCount;
};

class YETI_AesBlockEncrypter : public AesBlockCipher {
public:
    YETI_AesBlockEncrypter(const YETI_UInt8* key, YETI_Size key_size) :
      AesBlockCipher(key, key_size) {}
      Direction  get_direction() { return ENCRYPT; }
      YETI_Result process_block(const YETI_UInt8* block_in, YETI_UInt8* block_out);
};

class YETI_AesBlockDecrypter : public AesBlockCipher {
public:
    YETI_AesBlockDecrypter(const YETI_UInt8* key, YETI_Size key_size) :
      AesBlockCipher(key, key_size) {}
      Direction  get_direction() { return DECRYPT; }
      YETI_Result process_block(const YETI_UInt8* block_in, YETI_UInt8* block_out);
};

AesBlockCipher::AesBlockCipher(const YETI_UInt8* key, 
                                         YETI_Size         key_size)
{
    unsigned int i, j;
    YETI_UInt32 temp;
...
YETI_Result YETI_AesBlockEncrypter::process_block(const YETI_UInt8* pt, YETI_UInt8* ct)
{
    YETI_UInt32 s0, s1, s2, s3, t0, t1, t2, t3;
    int Nr = m_RoundCount;
    YETI_UInt32* rk = m_eK;
    ...
    */
    s0 =
        (Te4_3[YETI_BYTE(t0, 3)]) ^
        (Te4_2[YETI_BYTE(t1, 2)]) ^
        (Te4_1[YETI_BYTE(t2, 1)]) ^
        (Te4_0[YETI_BYTE(t3, 0)]) ^
        rk[0];
    STORE32H(s0, ct);
    s1 =
        (Te4_3[YETI_BYTE(t1, 3)]) ^
        (Te4_2[YETI_BYTE(t2, 2)]) ^
        (Te4_1[YETI_BYTE(t3, 1)]) ^
        (Te4_0[YETI_BYTE(t0, 0)]) ^
        rk[1];
    STORE32H(s1, ct+4);
    s2 =
        (Te4_3[YETI_BYTE(t2, 3)]) ^
        (Te4_2[YETI_BYTE(t3, 2)]) ^
        (Te4_1[YETI_BYTE(t0, 1)]) ^
        (Te4_0[YETI_BYTE(t1, 0)]) ^
        rk[2];
    STORE32H(s2, ct+8);
    s3 =
        (Te4_3[YETI_BYTE(t3, 3)]) ^
        (Te4_2[YETI_BYTE(t0, 2)]) ^
        (Te4_1[YETI_BYTE(t1, 1)]) ^
        (Te4_0[YETI_BYTE(t2, 0)]) ^ 
        rk[3];
    STORE32H(s3, ct+12);
...
YETI_Result YETI_AesBlockDecrypter::process_block(const YETI_UInt8* ct, YETI_UInt8* pt)
{
    YETI_UInt32 s0, s1, s2, s3, t0, t1, t2, t3;
    ...
    */
    s0 =
        (Td4[YETI_BYTE(t0, 3)] & 0xff000000) ^
        (Td4[YETI_BYTE(t3, 2)] & 0x00ff0000) ^
        (Td4[YETI_BYTE(t2, 1)] & 0x0000ff00) ^
        (Td4[YETI_BYTE(t1, 0)] & 0x000000ff) ^
        rk[0];
    STORE32H(s0, pt);
    s1 =
        (Td4[YETI_BYTE(t1, 3)] & 0xff000000) ^
        (Td4[YETI_BYTE(t0, 2)] & 0x00ff0000) ^
        (Td4[YETI_BYTE(t3, 1)] & 0x0000ff00) ^
        (Td4[YETI_BYTE(t2, 0)] & 0x000000ff) ^
        rk[1];
    STORE32H(s1, pt+4);
    s2 =
        (Td4[YETI_BYTE(t2, 3)] & 0xff000000) ^
        (Td4[YETI_BYTE(t1, 2)] & 0x00ff0000) ^
        (Td4[YETI_BYTE(t0, 1)] & 0x0000ff00) ^
        (Td4[YETI_BYTE(t3, 0)] & 0x000000ff) ^
        rk[2];
    STORE32H(s2, pt+8);
    s3 =
        (Td4[YETI_BYTE(t3, 3)] & 0xff000000) ^
        (Td4[YETI_BYTE(t2, 2)] & 0x00ff0000) ^
        (Td4[YETI_BYTE(t1, 1)] & 0x0000ff00) ^
        (Td4[YETI_BYTE(t0, 0)] & 0x000000ff) ^
        rk[3];
    STORE32H(s3, pt+12);
...
YETI_Result BlockCipher::create(Algorithm         algorithm, 
                         Direction         direction,
                         ...
                         BlockCipher*& cipher)
{
    ...
    switch (algorithm) {
case AES_128: 
    if (key == NULL || key_size != 16) return YETI_ERROR_INVALID_PARAMETERS;
    switch (direction) {
case ENCRYPT: 
    cipher = new YETI_AesBlockEncrypter(key, 16);
    return YETI_SUCCESS;

case DECRYPT:
    cipher = new YETI_AesBlockDecrypter(key, 16);
    return YETI_SUCCESS;
...
YETI_Result BlockCipher::process_cbc(const YETI_UInt8* input, YETI_Size input_size, const YETI_UInt8* iv, DataBuffer& output)
{
    // this version only supports block sizes of 16
    if (get_block_size() != 16) return YETI_ERROR_NOT_SUPPORTED;

    // decrypt or encrypt depending on the direction of the cipher
    YETI_UInt8 chain[16];
    if (iv) {
        MemoryCopy(chain, iv, 16);
    } else {
        SetMemory(chain, 0, 16);
    }
    if (get_direction() == ENCRYPT) {
        // pad
        unsigned int padding_size = 16-(input_size%16);
        DataBuffer padded_input;
        padded_input.set_data_size(input_size+padding_size);
        YETI_UInt8* plaintext = padded_input.use_data();
        MemoryCopy(plaintext, input, input_size);
        ...
        output.set_data_size(block_count*16);
        YETI_UInt8* ciphertext = output.use_data();
        for (unsigned int x=0; x<block_count; x++) {
            ...
            // encrypt the block
            YETI_Result result = process_block(plaintext, ciphertext);
            if (YETI_FAILED(result)) return result;

            // chain and move forward to the next block
            MemoryCopy(chain, ciphertext, 16);
            plaintext  += 16;
            ciphertext += 16;
        }
    } else {
        // check that we have an integral number of blocks
        if (input_size%16) return YETI_ERROR_INVALID_PARAMETERS;

        // process all blocks
        unsigned int block_count = input_size/16;
        output.set_buffer_size(block_count*16);
        YETI_UInt8* plaintext = output.use_data();
        const YETI_UInt8* ciphertext = input;
        for (unsigned int x=0; x<block_count; x++) {
            // decrypt block
            YETI_Result result = process_block(ciphertext, plaintext);
            if (YETI_FAILED(result)) return result;
        ...
        output.set_data_size(block_count*16 - padding_size);
    }
