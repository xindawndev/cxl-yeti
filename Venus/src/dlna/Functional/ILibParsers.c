...
int ILibHashTreeIsEmpty( void * tree )
{
    return ( ( ((struct HashNode_Root*)tree)->Root->Next == ((struct HashNode_Root*)tree)->Root->Prev )  && \
        ( ((struct HashNode_Root*)tree)->Root->Next == NULL ) )? 1 : 0;
}
...
int ILibInPlaceXmlUnEscape(char* data)
{
    ...
    while (j < end)
    {
        ...
        else
        {
            ...
            else if (j[0] == '&' && j[1] == 'q' && j[2] == 'u' && j[3] == 'o' && j[4] == 't' && j[5] == ';')   // &quot;
            {
                // Double Quote
                i[0] = '"';
                j += 5;
